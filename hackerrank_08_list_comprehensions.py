...
# You have to print a list of all possible 
# coordinates given by (i,j,k) on a 3D grid where the sum 
# of i + j + k is not equal to N. Here, 
# 0 <= i <= X;
# 0 <= j <= Y;
# 0 <= k <= Z
...
# List comprehensions are an elegant way to build 
# a list without having to use different for loops 
# to append values one by one. This example might help.
...
# Other smaller codes may also exist, but 
# using list comprehensions is always a good option. Code using list comprehensions:
# python 
# x = int ( raw_input()) 
# y = int ( raw_input()) 
# n = int ( raw_input()) 
# print [ [ i, j] for i in range( x + 1) 
# for j in range( y + 1) 
# if ( ( i + j ) != n )]
